...
func NewStandardThriftServer(addr string, opt *Option, processor thrift.TProcessor) (*thrift.TSimpleServer, error) {
	printStatus(addr, opt)
	protocolFactory, ok := protocolFactoryMap[opt.Protocol]
	if !ok {
		return nil, fmt.Errorf("no protocol found for NewThriftServer %s", opt.Protocol)
	}
